...
function parseUserAgent(uaString: string) {
  ...
  let device = "desktop";
  if (
    /mobile|touch|iphone|ipad|ipod|android|blackberry|iemobile|opera mini/i.test(
      ua,
    )
  ) {
...
export async function trackVisit(req: Request, res: Response): Promise<void> {
  ...
  if (ip !== "127.0.0.1" && ip !== "::1" && ip !== "unknown") {
    ...
    const num =
      Math.abs(
        ip.split(".").reduce((acc, octet) => acc + parseInt(octet, 10), 0),
      ) % 5;
    const cities = [
      "New Delhi",
      "Bhopal",
  ...
  const visitorId =
    existingVisitorId || "vis_" + Math.random().toString(36).substr(2, 9);
  ...
  try {
    ...
    if (isConnected) {
      try {
        ...
        console.error(
          "⚠️ Database write failed during trackVisit, falling back to in-memory store:",
          dbErr?.message || dbErr,
        );
      }
    }
...
export async function updateSession(
  ...
  const { sessionToken, activePage, pagesVisited, duration, exitTime } =
    req.body;
  ...
  try {
    ...
    if (isConnected) {
      try {
        ...
        if (pagesVisited) updateData.pagesVisited = pagesVisited;

        // Use findOneAndUpdate for atomic operations to avoid version conflicts
        const session = await (SessionModel as any).findOneAndUpdate(
          { sessionToken },
          updateData,
        ...
        if (!session) {
          console.warn("⚠️ Session not found:", sessionToken);
        }
        processedDb = true;
      } catch (dbErr: any) {
        console.error(
          "⚠️ Database error during updateSession, falling back to in-memory store:",
          dbErr?.message || dbErr,
        );
      }
    }
  ...
  try {
    ...
    if (isConnected) {
      try {
        ...
        console.error(
          "⚠️ Database reading error in getAnalyticsDashboard, falling back to in-memory store:",
          dbErr?.message || dbErr,
        );
      }
    }
    ...
    const totalDuration = sessionsList.reduce(
    ...
    const mostVisitedPages = Object.keys(pageHits)
      .map((name) => ({
        page: name,
        hits: pageHits[name],
      }))
    ...
    const trafficTimeline = Array.from(timelineMap.entries()).map(
      ([date, data]) => ({
        date,
        total: data.total,
        unique: data.unique,
    ...
    const avgRating =
      feedbacksList.length > 0
        ? Number(
            (
              feedbacksList.reduce((acc, f) => acc + f.rating, 0) /
              feedbacksList.length
            ).toFixed(1),
          )
        : 5.0;

    const recommendYes = feedbacksList.filter(
      (f) => String(f.recommend).toLowerCase() === "yes",
    ).length;
    ...
    res.json({
      ...
      locations: Object.keys(locations).map((l) => ({
        name: l,
        value: locations[l],
      })),
      mostVisitedPages,
      ...
      feedback: {
        ...
        scores: [
          {
            score: 5,
            count: feedbacksList.filter((f) => f.rating === 5).length,
          },
          {
            score: 4,
            count: feedbacksList.filter((f) => f.rating === 4).length,
          },
          {
            score: 3,
            count: feedbacksList.filter((f) => f.rating === 3).length,
          },
          {
            score: 2,
            count: feedbacksList.filter((f) => f.rating === 2).length,
          },
          {
            score: 1,
            count: feedbacksList.filter((f) => f.rating === 1).length,
          },
        ],
      },
    });
